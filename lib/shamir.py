...
Called from Bash via subprocess for split and reconstruct operations.
"""
import sys
import json
import secrets
...
def _gf_mul(a, b):
# Multiplication in GF(2^8) via Russian peasant algorithm
result = 0
for _ in range(8):
if b & 1:
result ^= a
...
Returns list of (x, y_bytes) tuples.
Each byte of the secret gets its own independent polynomial.
"""
shares = [[] for _ in range(n)]
for byte in secret_bytes:
# Polynomial of degree k-1, constant term = secret byte
coefficients = [byte] + [secrets.randbelow(256) for _ in range(k - 1)]
for i in range(n):
x = i + 1 # x values are 1..n, never 0 (0 is the secret)
y = _evaluate_polynomial(coefficients, x)
shares[i].append(y)
# Zero coefficients after use
...
print(secret.hex())
# Zero reconstructed secret immediately
